...
class Animal(Agent):
    ...
    def update_max_values(self):
        ...
        # print(current_time)
        if current_time != Animal.last_update:

            # print('update')
            Animal.max_prey = Animal.prey*(self.ALPHA)
            Animal.max_predators = Animal.predators*Animal.prey*self.DELTA
            Animal.last_update = current_time
            # print(cls.prey)
            # print(cls.max_prey)
...
# animal_config.alpha = params[0]
# animal_config.beta = params[1]
# animal_config.delta = params[2]
# animal_config.gamma = params[3]




# sim = Simulation(Config(movement_speed=1, seed=1, radius=12, duration=DURATION))
# sim.batch_spawn_agents(110, animal_agent_partial, ["images/prey.png", "images/predator.png"])
# # sim.run()
# try:
...
#     plt.savefig('Assignment2/energy/predator_prey_energy_2401.png')
# except:
#     print('population deceased')
...
def run_simulation(params):
    ...
    def create_animal(images, simulation):
        animal = Animal(images, simulation)
        animal.ALPHA = params[0]
        animal.BETA= params[1]
        animal.DELTA = params[2]
        animal.GAMMA = params[3]
        return animal
    
    sim = Simulation(Config(movement_speed=1, seed=1, radius=12, duration=DURATION))
    sim.batch_spawn_agents(110, create_animal, ["images/prey.png", "images/predator.png"])
    
    try:
        df = ((sim.run().snapshots))
        df = df.to_pandas()

        # Get highest frame for each image_index
        max_frame = df['frame'].max()
        final_populations = df[df['frame'] == max_frame].groupby('image_index').size()

        if 0 not in final_populations or final_populations[0] <= 0:  # All prey dead
            return "All prey dead"
        elif 1 not in final_populations or final_populations[1] <= 0:  # All predator dead
            return "All predator dead"
        else:  # Both species still alive
            return "Stable/Cyclic"
    except: 
        return "Ecosystem deceased"


outcome = run_simulation(params_list[7])
print(outcome)
print(params_list[7])
print()
...
# results_df = pd.DataFrame(results, columns=['Params', 'Outcome'])
# print(results_df)
