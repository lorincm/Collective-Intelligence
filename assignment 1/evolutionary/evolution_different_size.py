...
class SimulationFitness(object):
    ...
    def split_params(self, params):
        ...
        self.params = params
        print(self.params)
        self.tjoin=int(self.params.tolist()[0])
        self.tleave=int(self.params.tolist()[1])
        self.radius=int(self.params.tolist()[2])

    def simulation(self, i):
        cockroach_config = CockroachConfig()
        cockroach_config.tjoin = self.tjoin
        cockroach_config.tleave = self.tleave

        cockroach_agent_partial = partial(Cockroach, config=cockroach_config)
        
        sim=HeadlessSimulation(Config(movement_speed=5, duration=self.DURATION, seed=i, radius=self.radius))
...
class CockroachEA(object):
    ...
    def parent_selection(self, x_old, f_old):
        ...
        f_parents, x_parents = f_parents[0:50], x_ # @classmethodparents[0:50]
...
class Cockroach(Agent):
    ...
    def change_position(self):
        ...
        if self.state == State.WANDERING:
            ...
            if ((185 < self.pos[0] < 265) and (335 < self.pos[1] < 415)) or ((435 < self.pos[0] < 615) and (285 < self.pos[1] < 465)):     
...
def plot_evolution(x_best, f_best):
    ...
    sns.lineplot(data=data, x='Generation', y='tjoin', label='tjoin')
    sns.lineplot(data=data, x='Generation', y='tleave', label='tleave')
    sns.lineplot(data=data, x='Generation', y='radius', label='radius')
    plt.legend()
